/*****************************************************************
 File: chull4.cc
 Purpose:
	This is a variant of chull.cc
	(read the leading comments in that file).

	The present version aims to adapt our input and outpoint
	point formats to conform to LEDA's convex hull examples
	(we want to take advantage of its window displays).
 ...
 Usage:
  	% chull4 [-l | -s] < input_file >! output_file
...
void	ReadVertices( void )
{
   ...
   int	    vNum = 0;
    ...
    while (std::cin)
    {
      ...
      int rate;
      std::cin >> ch1 >> x >> ch2 >> y >> ch3 >> z >> ch4 >> rate >> ch5;
      v = MakeNullVertex();
      v->v[X] = x;
      v->v[Y] = y;
      v->v[Z] = z;
