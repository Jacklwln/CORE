...
public:
    ...
    void updateStatus() {
        ...
        if (corners.size() == 0 && walls.size() == 0){
            ...
            if(dupW){
                if (mindistW > rB && nearestW != NULL && !(nearestW->isRight(this->x, this->y)) && nearestW2 != NULL && !(nearestW2->isRight(this->x, this->y))){
                    status = STUCK;
                ...
                if (mindistW > rB && nearestW != NULL && !(nearestW->isRight(this->x, this->y))){
                    status = STUCK;
